...
@section('content')
    <div id="pidDiv" class="row form-inline row-align-bottom">
        ...
        <div class="form-group col-sm-4 group-align-bottom">
            ...
            {!! Form::button("增加", ['id' => 'addItem','class' => 'btn btn-primary']) !!}
            {!! Form::button("导入产品数据", ['id' => 'loadData','class' => 'btn btn-primary']) !!}
        </div>
    </div>
                        ...
                        <div>
                            <form class="form-horizontal">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-4">
                                        {!! Form::select('add_color',array(''=>''), null, array('id'=>'add_color')) !!}
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label for="add_size" class="col-sm-2 control-label">尺寸</label>
                                    <div class="col-sm-4">
                                        {!! Form::select('add_size',array(''=>''), null, array('id'=>'add_size')) !!}
                                    </div>
                                </div>
                                <div class="form-group">
    ...
    $(function () {
        ...
        $('#add_prd_id').keydown(function (event){
            if (event.keyCode == "13") {
                ...
                $.get('{{ route('products.productdetails') }}', {'product_id': $('#add_prd_id').val()}, function(data, status) {
                    console.log(data);
        ...
        $('#loadData').on('click',function() {
            ...
            $.ajax({
                url: '{{ route('storages.importproductdata') }}',
                type: "get",
                success: function(data) {
                    if (oTable != null) {
                        oTable.draw();
